...
- (void)initSubViews {
    ...
    _buttonBackgroundView.backgroundColor = [UIColor clearColor];
    [self addSubview:_buttonBackgroundView];
    
    if (self.title.length) {
        UILabel *titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, ButtonHeight * kHMulriple-TitleHeight * kHMulriple, ScreenWidth, ButtonHeight * kHMulriple)];
    ...
    UIView *view = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 355 * kMulriple, 120.5 * kHMulriple)];
    view.backgroundColor = [UIColor whiteColor];
    view.layer.cornerRadius = 20 * kMulriple;
    view.layer.masksToBounds = YES;
    [_buttonBackgroundView addSubview:view];
    ...
    for (int i = 0; i < _otherButtonTitles.count; i++) {
        ...
        button.titleLabel.font = [UIFont systemFontOfSize:20 * kMulriple];
        button.frame = CGRectMake(20 * kMulriple, 60.5 * kHMulriple * i, 315 * kMulriple, 60 * kHMulriple);
        [_buttonBackgroundView addSubview:button];
        
        
        UIView *line = [[UIView alloc] initWithFrame:CGRectZero];
        line.backgroundColor = RGB(200, 200, 200);
        line.frame = CGRectMake(0, 60 * kHMulriple, ScreenWidth, 0.5);
    ...
    [cancelButton setTitleColor:RGB(0, 122, 255) forState:UIControlStateNormal];
    [cancelButton addTarget:self action:@selector(cancelButton_Click:) forControlEvents:UIControlEventTouchUpInside];
    cancelButton.titleLabel.font = [UIFont systemFontOfSize:20 * kMulriple];
    cancelButton.frame = CGRectMake(0, 135 * kHMulriple, 355 * kHMulriple, 60 * kHMulriple);
    [_buttonBackgroundView addSubview:cancelButton];
    
    cancelButton.layer.cornerRadius = 20 * kMulriple;
    cancelButton.layer.masksToBounds = YES;
    _buttonBackgroundView.frame = CGRectMake(10 * kMulriple, 652 * kHMulriple, 355 * kMulriple, 190.5 * kHMulriple);
