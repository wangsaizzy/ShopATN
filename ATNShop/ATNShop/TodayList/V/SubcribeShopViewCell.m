...
@implementation SubcribeShopViewCell
...
- (UIImageView *)posterImageView {
    if (!_posterImageView) {
        self.posterImageView = [[UIImageView alloc] initWithFrame:CGRectMake(75 * kMulriple, 15 * kHMulriple, 50 * kMulriple, 50 * kHMulriple)];
        _posterImageView.image = [UIImage imageNamed:@"通用-用户默认头像_120"];
        _posterImageView.layer.cornerRadius = 25 * kMulriple;
...
- (UILabel *)subLabel {
    if (!_subLabel) {
        self.subLabel = [[UILabel alloc] initWithFrame:CGRectMake(140 * kMulriple, 40 * kHMulriple, 90 * kMulriple, 30 * kHMulriple)];
        _subLabel.text = @"预约";
        _subLabel.textColor = RGB(111, 111, 111);
        _subLabel.font = [UIFont systemFontOfSize:15 * kMulriple];
    }
    return _subLabel;
}
...
- (UILabel *)nameLabel {
    if (!_nameLabel) {
        self.nameLabel = [[UILabel alloc] initWithFrame:CGRectMake(140 * kMulriple, 20 * kHMulriple, 230 * kMulriple, 15 * kHMulriple)];
        _nameLabel.textColor = RGB(111, 111, 111);
        _nameLabel.font = [UIFont systemFontOfSize:19 * kMulriple];
    }
    return _nameLabel;
}
...
- (void)setModel:(SubscribeModel *)model {
    ...
    NSString *portrait = [NSString stringWithFormat:@"%@", model.userPortrait];
    
    [self.posterImageView sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@", Service_Url, portrait]] placeholderImage:[UIImage imageNamed:@"userImage"]];
}
