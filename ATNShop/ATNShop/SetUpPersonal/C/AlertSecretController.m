...
#import "LoginSecretCodeView.h"
@interface AlertSecretController ()<UIGestureRecognizerDelegate>

{
    LoginSecretCodeView *_testView;
}
...
- (void)setupViews {
    _testView = [[LoginSecretCodeView alloc] initWithFrame:self.view.bounds];
    self.view = _testView;
...
- (void)commitBtnAction:(UIButton *)sender {
    ...
    NSLog(@"%@", [UserModel defaultModel].phone);

    if (![_testView.phoneNumberTF.text isEqualToString:[UserModel defaultModel].phone]) {
        
        ShowAlertView(@"请输入绑定手机号");
        return;
    }
